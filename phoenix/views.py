...
def home(request):
    ...
    if request.user.is_authenticated:
        try:
            ...
            panier_items = Panier.objects.filter(profil=profil)
...
def blog_posts(request):
    ...
    return render(request, 'blog.html', {'articles': articles, 'nombre_produits_panier': nombre_produits_panier})

def admin_pannele(request):
    return render(request, 'admin_pannele.html')
...
def signin(request):
    if request.method == 'POST':
        ...
        if user is not None:
            login(request, user)
            return redirect('home')  # Remplacez 'home' par la route souhaitée après connexion
        else:
...
def ajouter_au_panier(request, produit_id):
    ...
    if not request.user.is_authenticated:
        return redirect('signin')
    produit = get_object_or_404(Produit, id=produit_id)
    profil = Profil.objects.get(user=request.user)  # Associe le panier au profil de l'utilisateur
    
    # Vérifie si le produit est déjà dans le panier
    panier, created = Panier.objects.get_or_create(profil=profil, produit=produit)
...
def afficher_panier(request):
    ...
    profil = Profil.objects.get(user=request.user)
    paniers = Panier.objects.filter(profil=profil)
...
def supprimer_du_panier(request, produit_id):
    ...
    panier = get_object_or_404(Panier, profil=profil, produit_id=produit_id)

    # Supprimez l'élément du panier
    panier.delete()

    return redirect('afficher_panier')  # Redirige vers la vue du panier
